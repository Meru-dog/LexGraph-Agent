...
def _run_review_agent(task_id: str, raw_contract: str, request: ReviewRequest) -> None:
    ...
    try:
        ...
        for step_output in review_graph.stream(initial_state, config=config):
            ...
            _tasks.update(task_id, {"current_node": node_name})

        state = review_graph.get_state(config)
        _tasks.update(task_id, {
            "status": "complete",
            "original_text": raw_contract,
            "reviewed_text": state.values.get("redlined_contract", ""),
            "clause_reviews": state.values.get("clause_reviews", []),
            "compliance_flags": state.values.get("compliance_flags", []),
        })
...
@router.get("")
async def list_review_tasks(current_user: User = Depends(get_current_user)):
